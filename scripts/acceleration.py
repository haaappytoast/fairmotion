...
class Acceleration(object):
    ...
    def compute(cls, v_init, v_final, dt):
        ...
        # for joint in range(0, np.array(v_final.data_local).shape[0]):
        #     angular_init = v_init.data_local[joint, 0:3]
        #     angular_final = v_final.data_local[joint, 0:3]
        #     linear_init = v_init.data_local[joint, 3:6]            
        #     linear_final = v_final.data_local[joint, 3:6]
            
        #     angular_acc = (angular_final - angular_init) / dt
        #     linear_acc = (linear_final - linear_init) / dt            
        #     data_local.append(np.hstack((angular_acc, linear_acc)))
            
        #     angular_init = v_init.data_global[joint, 0:3]
        #     angular_final = v_final.data_global[joint, 0:3]
        #     linear_init = v_init.data_global[joint, 3:6]            
        #     linear_final = v_final.data_global[joint, 3:6]
            
        #     angular_acc = (angular_final - angular_init) / dt
        #     linear_acc = (linear_final - linear_init) / dt            
        #     data_global.append(np.hstack((angular_acc, linear_acc)))
